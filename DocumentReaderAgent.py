...
def extract_all_images_to_base64(doc_path: str) -> Dict[str, str]:
    """
...
def extract_text_and_images_tool(doc_path: str) -> tuple[List[Dict[str, Union[str, List[str]]]], Dict[str, str]]:
    """
    ...
    all_images_data = extract_all_images_to_base64(doc_path) 
...
def summarize_text_tool(context_text: str) -> str:
...
def explain_image_tool(image_base64: str, context_summary: str) -> str:
...
# ==================== 4. ADK Tool Wrappers & Agent Definition ====================

extract_doc_tool_adk = FunctionTool(extract_text_and_images_tool)
summarize_doc_tool_adk = FunctionTool(summarize_text_tool)
explain_image_doc_tool_adk = FunctionTool(explain_image_tool)
clean_doc_tool_adk = FunctionTool(clean_text_tool)
...
# ==================== 5. Runner Function (Manual Orchestration) ====================

def process_word_doc_from_ui(doc_path: str) -> str:
    """
    ...
    all_chunks, all_images_data = extract_text_and_images_tool(doc_path)
    ...
    for idx, chunk in enumerate(all_chunks):
        ...
        if chunk["image_references"]:
            ...
            if image_name in all_images_data:
                ...
                img_explanation = explain_image_tool(image_base64, summary)
    ...
    # Clean the output (Function is called directly!)
    cleaned_output = clean_text_tool(full_raw_text)
