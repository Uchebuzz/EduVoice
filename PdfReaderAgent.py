...
def ocr_page_tool(pdf_path: str, page_number: int) -> str:
...
def explain_image_tool(image_bytes: bytes, context_summary: str) -> str:
...
# ==================== 3. ADK Tool Wrappers (for Agent use) ====================
summarize_tool_adk = FunctionTool(summarize_text_tool)
ocr_tool_adk = FunctionTool(ocr_page_tool)
explain_tool_adk = FunctionTool(explain_image_tool)
clean_tool_adk = FunctionTool(clean_text_tool)

# ==================== 4. PDF Reader Agent Definition ====================
...
# ==================== 5. Runner Function ====================
def process_pdf_from_ui(pdf_path: str) -> str:
    """
    ...
    for idx, chunk in enumerate(enhanced):
        ...
        if not text:
            # FIX: Call the original function ocr_page_tool
            text = ocr_page_tool(pdf_path, idx)
        
        chunk["text"] = text
        
        # FIX: Call the original function summarize_text_tool
        summary = summarize_text_tool(text) if text else None
    ...
    for idx, chunk in enumerate(enhanced):
        ...
        if images:
            ...
            # FIX: Call the original function explain_image_tool
            img_explanation = explain_image_tool(img_bytes, context)
    ...
    # ---------- PASS 3: Cleaning ----------
    combined = "\n".join(raw_text)
    # FIX: Call the original function clean_text_tool
    cleaned_output = clean_text_tool(combined)
